...
- `AGENTS.md` — agent workflow guide. This is the single source of truth for agent instructions; `.claude/CLAUDE.md` is a symlink to it.
- `/docs` — single source of truth for business requirements / design docs.

### Non-Claude agents

- `.mcp.json` — declares Cloudflare's hosted documentation MCP server (`https://docs.mcp.cloudflare.com/mcp`), so any MCP-capable agent (Cursor, Windsurf, Claude Code, etc.) answers platform questions from current Cloudflare docs instead of training-data memory.
- `.cursor/rules/agents-mirror.mdc` — a Cursor-format mirror of `AGENTS.md`, generated by `pnpm cursor-rules:sync`. It is **not** an independent source: `AGENTS.md` remains the single source of truth, this file is a regenerated copy of it, and `scripts/check-cursor-rules.test.ts` fails the test suite if the two drift out of sync.
